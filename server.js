...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Tentative d'insertion pour l'utilisateur ${email} avec le rôle: ${role}`
    );
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    const [users] = await connection.execute(
      "SELECT id, nom, prenom, email, motDePasse, role FROM Users WHERE email = ?",
      [email]
    );
    const user = users[0];
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!nom) {
      return res
        .status(400)
        .json({ message: "Le nom de la classe est requis." });
    }
    ...
    try {
      ...
      res.status(500).json({
        message: "Erreur interne du serveur lors de la création de la classe.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get("/api/classes/me", auth, async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Erreur lors de la récupération des classes de l'utilisateur :",
      error.message
    );
    res.status(500).json({ message: "Erreur interne du serveur." });
  } finally {
...
app.post(
  "/api/classes/join",
  auth,
  authorizeRole(["Etudiant", "Coordinateur"]),
  async (req, res) => {
    const { code } = req.body;
    ...
    if (!code) {
      return res
        .status(400)
        .json({ message: "Le code de la classe est requis." });
    }
    ...
    try {
      ...
      if (!classToJoin) {
        return res
          .status(404)
          .json({ message: "Classe introuvable avec ce code." });
      }
      ...
      if (existingEnrollment.length > 0) {
        return res
          .status(409)
          .json({ message: "Vous êtes déjà inscrit à cette classe." });
      }
      ...
      res.status(500).json({
        message: "Erreur interne du serveur lors de l'inscription à la classe.",
      });
    } finally {
      if (connection) connection.end();
    }
  ...
  async (req, res) => {
    ...
    if (!class_id || !titre || !contenu) {
      return res.status(400).json({
        message: "L'ID de la classe, le titre et le contenu sont requis.",
      });
    }
    ...
    try {
      ...
      const [classes] = await connection.execute(
        "SELECT id FROM Classes WHERE id = ? AND professeur_id = ?",
        [class_id, professeur_id]
      );
      if (classes.length === 0) {
        return res.status(403).json({
          message:
            "Vous n'êtes pas autorisé à publier une annonce dans cette classe.",
        });
      }
      ...
      res.status(500).json({
        message: "Erreur interne du serveur lors de la création de l'annonce.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get("/api/announcements/:classId", auth, async (req, res) => {
  ...
  try {
    ...
    if (enrollment.length === 0) {
      ...
      const [isProfessor] = await connection.execute(
        "SELECT id FROM Classes WHERE id = ? AND professeur_id = ?",
        [classId, userId]
      );
      if (isProfessor.length === 0) {
        return res
          .status(403)
          .json({ message: "Accès non autorisé à cette classe." });
      }
    }
    ...
    res.status(500).json({
      message:
        "Erreur interne du serveur lors de la récupération des annonces.",
    });
  } finally {
    if (connection) connection.end();
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!class_id || !titre) {
      return res
        .status(400)
        .json({ message: "L'ID de la classe et le titre sont requis." });
    }
    ...
    try {
      ...
      if (classes.length === 0) {
        return res.status(403).json({
          message:
            "Vous n'êtes pas autorisé à partager de la documentation dans cette classe.",
        });
      }
      ...
      res.status(500).json({
        message:
          "Erreur interne du serveur lors du partage de la documentation.",
      });
    } finally {
      if (connection) connection.end();
    }
  ...
  try {
    ...
    if (enrollment.length === 0) {
      ...
      if (isProfessor.length === 0) {
        return res
          .status(403)
          .json({ message: "Accès non autorisé à cette documentation." });
      }
    }
    ...
    console.error(
      "Erreur lors de la récupération de la documentation :",
      error.message
    );
    res.status(500).json({
      message:
        "Erreur interne du serveur lors de la récupération de la documentation.",
    });
  } finally {
    if (connection) connection.end();
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!class_id || !titre || !date_limite) {
      return res.status(400).json({
        message: "L'ID de la classe, le titre et la date limite sont requis.",
      });
    }
    ...
    try {
      ...
      if (classes.length === 0) {
        return res.status(403).json({
          message:
            "Vous n'êtes pas autorisé à assigner une tâche dans cette classe.",
        });
      }
      ...
      res.status(500).json({
        message: "Erreur interne du serveur lors de l'assignation de la tâche.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get("/api/tasks/class/:classId", auth, async (req, res) => {
  ...
  try {
    ...
    if (enrollment.length === 0 && isProfessor.length === 0) {
      return res
        .status(403)
        .json({ message: "Accès non autorisé à ces tâches." });
    }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!file_path && !content) {
      return res.status(400).json({
        message:
          "Un chemin de fichier ou un contenu est requis pour la soumission.",
      });
    }
    ...
    try {
      ...
      const [taskInfo] = await connection.execute(
        "SELECT class_id FROM Tasks WHERE id = ?",
        [taskId]
      );
      ...
      if (enrollment.length === 0) {
        return res.status(403).json({
          message:
            "Vous n'êtes pas autorisé à soumettre à cette tâche (non inscrit à la classe).",
        });
      }
      ...
      if (existingSubmission.length > 0) {
        ...
        res
          .status(200)
          .json({ message: "Soumission de tâche mise à jour avec succès !" });
      } else {
        // Nouvelle soumission
        ...
        res.status(201).json({
          message: "Tâche soumise avec succès !",
      ...
      res.status(500).json({
        message: "Erreur interne du serveur lors de la soumission de la tâche.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [taskInfo] = await connection.execute(
        "SELECT id FROM Tasks WHERE id = ? AND professeur_id = ?",
        [taskId, professeur_id]
      );
      if (taskInfo.length === 0) {
        return res.status(403).json({
          message:
            "Vous n'êtes pas autorisé à voir les soumissions de cette tâche.",
        });
      }
      ...
      res.status(500).json({
        message:
          "Erreur interne du serveur lors de la récupération des soumissions.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.put(
  ...
  async (req, res) => {
    ...
    if (grade === undefined || feedback === undefined) {
      return res
        .status(400)
        .json({ message: "La note et le feedback sont requis." });
    }
    ...
    try {
      ...
      if (
        ...
        submissionInfo[0].professeur_id !== professeur_id
      ) {
        return res.status(403).json({
      ...
      res
        .status(200)
        .json({ message: "Soumission notée et corrigée avec succès !" });
    } catch (error) {
      console.error(
        "Erreur lors de la notation de la soumission :",
        error.message
      );
      res.status(500).json({
        message:
          "Erreur interne du serveur lors de la notation de la soumission.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get("/api/submissions/:submissionId", auth, async (req, res) => {
  ...
  try {
    ...
    if (userRole === "Etudiant" && sub.student_id !== userId) {
      return res
        .status(403)
        .json({ message: "Vous n'êtes pas autorisé à voir cette soumission." });
    }
    ...
    console.error(
      "Erreur lors de la récupération de la soumission spécifique :",
      error.message
    );
    res.status(500).json({
      message:
        "Erreur interne du serveur lors de la récupération de la soumission.",
    });
  } finally {
    if (connection) connection.end();
  }
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        "Erreur lors de la récupération des soumissions de l'étudiant :",
        error.message
      );
      res.status(500).json({
        message:
          "Erreur interne du serveur lors de la récupération des soumissions de l'étudiant.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.post("/api/messages", auth, async (req, res) => {
  ...
  try {
    ...
    if (!content || content.trim() === "") {
      return res
        .status(400)
        .json({ message: "Le contenu du message ne peut pas être vide." });
    }
    if (!["public", "private"].includes(message_type)) {
      return res.status(400).json({
        message: 'Type de message invalide. Doit être "public" ou "private".',
      });
    }
    ...
    if (message_type === "private") {
      if (!receiver_id) {
        return res.status(400).json({
          message: "L'ID du destinataire est requis pour un message privé.",
        });
      }
      ...
      if (sender_id === receiver_id) {
        return res.status(400).json({
          message:
            "Vous ne pouvez pas vous envoyer un message privé à vous-même.",
        });
      }
    ...
    } else if (message_type === "public") {
      // message_type === 'public'
      if (!class_id) {
        return res.status(400).json({
          message: "L'ID de la classe est requis pour un message public.",
        });
      }
      ...
      if (classMembership.length === 0) {
        return res.status(403).json({
          message:
            "Vous devez être membre de cette classe pour y envoyer un message public.",
        });
      }
      // Optionnel : Vérifier si l'expéditeur a le rôle approprié pour envoyer des messages publics (Professeur ou Coordinateur)
      const [senderRole] = await connection.execute(
        "SELECT role FROM Users WHERE id = ?",
        [sender_id]
      );
      if (
        senderRole.length === 0 ||
        !["Professeur", "Coordinateur"].includes(senderRole[0].role)
      ) {
        return res.status(403).json({
          message:
            "Seuls les professeurs ou coordinateurs peuvent envoyer des messages publics.",
        });
      }
...
app.get("/api/messages/public/class/:classId", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message:
        "Erreur interne du serveur lors de la récupération des messages publics.",
    });
  } finally {
    if (connection) connection.end();
  }
...
app.get("/api/messages/private/me", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message:
        "Erreur interne du serveur lors de la récupération des messages privés.",
    });
  } finally {
    if (connection) connection.end();
  }
...
app.get(
  "/api/users/all",
  auth,
  authorizeRole(["Professeur", "Coordinateur"]),
  async (req, res) => {
    let connection;
    try {
      connection = await getConnection();
      const [users] = await connection.execute(
        "SELECT id, prenom, nom, email, role FROM Users ORDER BY role, nom, prenom"
      );
...
app.get("/api/classes/me", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message:
        "Erreur interne du serveur lors de la récupération de vos classes.",
    });
  } finally {
    if (connection) connection.end();
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (isClassOwner.length === 0 && req.user.role !== "Coordinateur") {
        return res.status(403).json({
          message:
            "Vous n'avez pas la permission de créer un projet pour cette classe.",
        });
      }
      if (req.user.role === "Coordinateur") {
        // Si coordinateur, vérifier qu'il est membre de la classe (s'il le faut)
        const [isMember] = await connection.execute(
          "SELECT * FROM ClassMembers WHERE class_id = ? AND user_id = ?",
          [class_id, user_id]
        );
        if (isMember.length === 0) {
          return res.status(403).json({
            message:
              "En tant que Coordinateur, vous devez être membre de la classe pour créer un projet.",
          });
        }
      }
...
app.get("/api/projects/class/:classId", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Erreur interne du serveur lors de la récupération des projets.",
    });
  } finally {
    if (connection) connection.end();
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (professeur_id !== user_id && req.user.role !== "Coordinateur") {
        return res.status(403).json({
          message:
            "Vous n'avez pas la permission de créer un groupe pour ce projet.",
        });
      }
      ...
      if (req.user.role === "Coordinateur") {
        ...
        if (isMember.length === 0) {
          return res.status(403).json({
            message:
              "En tant que Coordinateur, vous devez être membre de la classe du projet pour créer un groupe.",
          });
        }
      }
      ...
      res.status(500).json({
        message: "Erreur interne du serveur lors de la création du groupe.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        "Erreur lors de la reccupération des groupes :",
        error.message
      );
      res.status(500).json({
        message:
          "Erreur interne du serveur lors de la reccupération des groupes",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get("/api/groups/project/:projectId", auth, async (req, res) => {
  ...
  try {
    ...
    if (isMember.length === 0) {
      return res
        .status(403)
        .json({ message: "Accès non autorisé à ce projet ou à sa classe." });
    }
    ...
    console.error(
      "Erreur lors de la récupération des groupes du projet :",
      error.message
    );
    res.status(500).json({
      message: "Erreur interne du serveur lors de la récupération des groupes.",
    });
  } finally {
    if (connection) connection.end();
  }
    ...
    try {
      ...
      if (req.user.role === "Coordinateur") {
        ...
        if (isMember.length === 0) {
          return res.status(403).json({
            message:
              "En tant que Coordinateur, vous devez être membre de la classe du projet du groupe pour le modifier.",
          });
        }
      }
      ...
      if (userToAddInfo.length === 0 || userToAddInfo[0].role !== "Etudiant") {
        return res.status(400).json({
          message:
            "L'utilisateur à ajouter n'existe pas ou n'est pas un étudiant.",
        });
      }
      ...
      if (isStudentInClass.length === 0) {
        return res.status(400).json({
          message: "L'étudiant n'est pas membre de la classe de ce projet.",
        });
      }
      ...
      if (existingMember.length > 0) {
        return res
          .status(409)
          .json({ message: "Cet utilisateur est déjà membre de ce groupe." });
      }
      ...
      console.error(
        "Erreur lors de l'ajout d'un membre au groupe :",
        error.message
      );
      res.status(500).json({
        message: "Erreur interne du serveur lors de l'ajout du membre.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Membre supprimé du groupe avec succès." });
    } catch (error) {
      console.error(
        "Erreur lors de la suppression d'un membre du groupe :",
        error.message
      );
      res.status(500).json({
        message: "Erreur interne du serveur lors de la suppression du membre.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.get("/api/classes/:classId/students", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message:
        "Erreur interne du serveur lors de la récupération des étudiants de la classe.",
    });
  } finally {
    if (connection) connection.end();
  }
...
app.put(
  ...
  async (req, res) => {
    ...
    if (parseInt(userId) === requestingUserId && role !== "Coordinateur") {
      return res.status(403).json({
        message: "Un Coordinateur ne peut pas se rétrograder lui-même.",
      });
    }
    ...
    try {
      ...
      res.status(200).json({
        message: `Rôle de l'utilisateur ${userId} mis à jour en "${role}".`,
      });
    } catch (error) {
      console.error(
        `Erreur lors de la modification du rôle de l'utilisateur ${userId} :`,
        error.message
      );
      res.status(500).json({
        message: "Erreur interne du serveur lors de la modification du rôle.",
      });
    } finally {
      if (connection) connection.end();
    }
...
app.delete(
  ...
  async (req, res) => {
    ...
    if (parseInt(userId) === requestingUserId) {
      return res.status(403).json({
        message: "Un Coordinateur ne peut pas supprimer son propre compte.",
      });
    }
    ...
    try {
      ...
      if (userToDelete.length > 0 && userToDelete[0].role === "Coordinateur") {
        const [allCoordinators] = await connection.execute(
          "SELECT COUNT(*) AS count FROM Users WHERE role = 'Coordinateur'"
        );
        if (allCoordinators[0].count === 1) {
          return res.status(403).json({
            message: "Impossible de supprimer le seul compte Coordinateur.",
          });
        }
      }
      ...
      res
        .status(200)
        .json({ message: `Utilisateur ${userId} supprimé avec succès.` });
    } catch (error) {
      console.error(
...
const checkIsGroupCoordinator = async (req, res, next) => {
  ...
  try {
    ...
    if (rows.length > 0 && rows[0].isGroupCoordinator) {
      ...
      res.status(403).json({
        message: "Accès refusé. Seul le responsable de ce groupe est autorisé.",
      });
    }
  } catch (error) {
    console.error(
      "Erreur lors de la vérification du rôle de responsable de groupe:",
      error.message
    );
    res.status(500).json({ message: "Erreur interne du serveur." });
  } finally {
